...
module Athenian
  ...
  class ReleaseMetricsRequest
    # List of repository groups for which to calculate the metrics. An empty repository (empty array) group will match nothing. Use `null` as repository group to match every repository. 
    attr_accessor :_for
    ...
    def self.openapi_types
      {
        :'_for' => :'Array<ReleaseMetricsRequestForInner>',
        :'with' => :'Array<ReleaseWith>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, 'The input argument (attributes) must be a hash in `Athenian::ReleaseMetricsRequest` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Athenian::ReleaseMetricsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [_for, with, labels_include, labels_exclude, jira, jiragroups, metrics, date_from, date_to, timezone, granularities, quantiles, account].hash
    end
