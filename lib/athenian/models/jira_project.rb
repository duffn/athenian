  ...
  class JIRAProject
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, 'The input argument (attributes) must be a hash in `Athenian::JIRAProject` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Athenian::JIRAProject`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'issues_count')
        self.issues_count = attributes[:'issues_count']
      end
    ...
    def list_invalid_properties
      ...
      if @issues_count.nil?
        invalid_properties.push('invalid value for "issues_count", issues_count cannot be nil.')
      end
    ...
    def hash
      [name, key, id, avatar_url, enabled, issues_count, last_update].hash
    end
