...
module Athenian
  # Response of `/metrics/developers`. The dates start from `date_from` and end earlier or equal to `date_to`. The order of the items is *not* guaranteed to match any expectations. 
  class CalculatedDeveloperMetrics
    ...
    def self.openapi_types
      {
        :'calculated' => :'Array<CalculatedDeveloperMetricsItem>',
        :'metrics' => :'Array<DeveloperMetricID>',
        :'date_from' => :'Date',
        :'date_to' => :'Date',
        :'timezone' => :'Integer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, 'The input argument (attributes) must be a hash in `Athenian::CalculatedDeveloperMetrics` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Athenian::CalculatedDeveloperMetrics`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [calculated, metrics, date_from, date_to, timezone, granularities].hash
    end
