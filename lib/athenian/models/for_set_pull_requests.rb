...
module Athenian
  # Filter for `/metrics/pull_requests` and `/histograms/pull_requests`. Achieve the best performance by packing all your filters in a single `ForSetPullRequests` instead of sending multiple `ForSetPullRequests`-s. For example, use `repogroups` and `withgroups` to request separate metrics for several repository sets and teams. 
  class ForSetPullRequests
    ...
    # Aggregate by groups of PR contributors. Currently, only the groupings by `author`, `merger`, and `releaser` are supported. People do not have to be mentioned in `with` in contrast to `repogroups`. We treat `with` as another group, if specified.
    attr_accessor :withgroups

    # At least one of these labels must be present in each analyzed PR.
    attr_accessor :labels_include

    # None of these labels must be present in each analyzed PR.
    attr_accessor :labels_exclude

    # Split by changed number of lines (inserted + removed).
    attr_accessor :lines

    attr_accessor :jira

    # Calculate metrics separately for each JIRA Filter in the list. Not compatible with `jira` field. 
    attr_accessor :jiragroups

    # Calculate deployment metrics separately for each environment in the list. Required if any deployment metrics were requested. Histograms are only implemented for single environments.
    ...
    def self.openapi_types
      {
        ...
        :'withgroups' => :'Array<PullRequestWith>',
        :'labels_include' => :'Array<String>',
        :'labels_exclude' => :'Array<String>',
        :'lines' => :'Array<Integer>',
        :'jira' => :'JIRAFilter',
        :'jiragroups' => :'Array<JIRAFilter>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, 'The input argument (attributes) must be a hash in `Athenian::ForSetPullRequests` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Athenian::ForSetPullRequests`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'jiragroups')
        if (value = attributes[:'jiragroups']).is_a?(Array)
          self.jiragroups = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if !@environments.nil? && @environments.length > 64
        invalid_properties.push('invalid value for "environments", number of items must be less than or equal to 64.')
      end
    ...
    def valid?
      ...
      return false if !@environments.nil? && @environments.length > 64
    ...
    def environments=(environments)
      if !environments.nil? && environments.length > 64
        fail ArgumentError, 'invalid value for "environments", number of items must be less than or equal to 64.'
      end
    ...
    def hash
      [repositories, repogroups, with, withgroups, labels_include, labels_exclude, lines, jira, jiragroups, environments].hash
    end
