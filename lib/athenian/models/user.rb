...
module Athenian
  ...
  class User
    ...
    # Identifier of the super admin user who is acting on behalf of.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, 'The input argument (attributes) must be a hash in `Athenian::User` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Athenian::User`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'native_id')
        self.native_id = attributes[:'native_id']
      end
    ...
    def list_invalid_properties
      ...
      if @native_id.nil?
        invalid_properties.push('invalid value for "native_id", native_id cannot be nil.')
      end
    ...
    def hash
      [accounts, email, id, impersonated_by, login, native_id, name, picture, updated].hash
    end
