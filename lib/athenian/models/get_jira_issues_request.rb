...
module Athenian
  ...
  class GetJIRAIssuesRequest
    ...
    def self.openapi_types
      {
        :'account' => :'Integer',
        :'include' => :'Array<GetJIRAIssuesInclude>',
        :'issues' => :'Array<String>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, 'The input argument (attributes) must be a hash in `Athenian::GetJIRAIssuesRequest` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Athenian::GetJIRAIssuesRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @issues.length < 1
        invalid_properties.push('invalid value for "issues", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @issues.length < 1
      true
    end
    ...
    def issues=(issues)
      ...
      if issues.length < 1
        fail ArgumentError, 'invalid value for "issues", number of items must be greater than or equal to 1.'
      end
